...
function handleLogin(data) {
    ...
    for (let i = 1; i < usersData.length; i++) {
        ...
        const rowEmail = userSchema.email !== undefined ? String(usersData[i][userSchema.email] || '').trim() : '';
        const rowPasswordHash = String(usersData[i][userSchema.passwordHash] || '').trim();

        if (rowUsername.toLowerCase() !== username.toLowerCase() &&
            (!rowEmail || rowEmail.toLowerCase() !== username.toLowerCase())) {
            continue;
        }
...
function handleRegisterUser(data) {
    ...
    if (!fullName || !username || (!password && !passwordHashInput) || !referralCodeInput || !emailInput) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)) {
        return JsonResponse({ success: false, message: 'Địa chỉ email không hợp lệ' });
    }
    ...
    try {
        ...
        for (let i = 1; i < usersData.length; i++) {
            ...
            // Kiểm tra trùng email (nếu cột email được định nghĩa)
            if (userSchema.email >= 0) {
                const existingEmail = String(usersData[i][userSchema.email] || '').trim();
                if (existingEmail && existingEmail.toLowerCase() === emailInput.toLowerCase()) {
                    return JsonResponse({ success: false, message: 'Email này đã được sử dụng bởi tài khoản khác' });
                }
            }
        }
        ...
        if (userSchema.email >= 0) {
            newUserRow[userSchema.email] = emailInput;
        }
...
function getUsersSchema_(usersSheet) {
    ...
    const email = findHeaderIndex_(normalizedHeaders, ['email', 'thudientu', 'mail'], -1);
...
function handleGetAccountInfo(data) {
    ...
    for (let i = 1; i < usersData.length; i++) {
        ...
        email = userSchema.email !== undefined ? String(usersData[i][userSchema.email] || '').trim() : '';
        break;
    }
...
function handleUpdateUserInfo(data) {
    ...
    // Validate new data
    const newFullName = String(data.fullName || '').trim();
    const newUsername = String(data.username || '').trim();
    const newEmail = String(data.email || '').trim();

    if (!newFullName || !newUsername) {
        return JsonResponse({ success: false, message: 'Họ tên và Tên đăng nhập không được để trống' });
    }
    ...
    // First pass: check uniqueness and find the user's row
    for (let i = 1; i < usersData.length; i++) {
        const rowUsername = String(usersData[i][userSchema.username] || '').trim();
        
        // Find the user's own row
        if (rowUsername.toLowerCase() === currentUsername.toLowerCase()) {
            targetRow = i + 1;
        } else {
            // Check uniqueness of username and email against other users
            if (rowUsername.toLowerCase() === newUsername.toLowerCase()) {
                return JsonResponse({ success: false, message: 'Tên đăng nhập này đã có người sử dụng' });
            }
            if (newEmail && userSchema.email !== undefined) {
                const rowEmail = String(usersData[i][userSchema.email] || '').trim();
                if (rowEmail.toLowerCase() === newEmail.toLowerCase()) {
                    return JsonResponse({ success: false, message: 'Email này đã được sử dụng bởi tài khoản khác' });
                }
            }
        }
    ...
    if (targetRow === -1) {
        return JsonResponse({ success: false, message: 'Không tìm thấy thông tin tài khoản' });
    }
    ...
    try {
        usersSheet.getRange(targetRow, userSchema.fullName + 1).setValue(newFullName);
        usersSheet.getRange(targetRow, userSchema.username + 1).setValue(newUsername);
        
        if (userSchema.email !== undefined) {
            usersSheet.getRange(targetRow, userSchema.email + 1).setValue(newEmail);
        }

        // Generate a new token if username changed
        let updatedToken = data.token;
        if (newUsername.toLowerCase() !== currentUsername.toLowerCase()) {
            updatedToken = generateToken(newUsername, token.role);
        }
        ...
        return JsonResponse({ success: false, message: 'Lỗi cập nhật: ' + error.toString() });
    } finally {
        lock.releaseLock();
    }
}
...
function handleRequestPasswordReset(data) {
    ...
    if (!username || !email) {
        return JsonResponse({ success: false, message: 'Vui lòng nhập tên đăng nhập và email.' });
    }
    ...
    if (userSchema.email < 0) {
        return JsonResponse({ 
            success: false, 
            message: 'Hệ thống chưa được cấu hình cột Email trong cơ sở dữ liệu. Vui lòng khôi phục bằng Mã giới thiệu hoặc liên hệ Admin.' 
        });
    }
    ...
    for (let i = 1; i < usersData.length; i++) {
        ...
        if (rowUsername.toLowerCase() === username.toLowerCase()) {
            matchedRowIndex = i;
            registeredEmail = String(usersData[i][userSchema.email] || '').trim();
            break;
        }
    }
    ...
    if (!registeredEmail) {
        return JsonResponse({ 
            success: false, 
            message: 'Tài khoản chưa được liên kết email. Vui lòng khôi phục bằng Mã giới thiệu hoặc liên hệ Admin.' 
        });
    }

    if (registeredEmail.toLowerCase() !== email.toLowerCase()) {
        return JsonResponse({ success: false, message: 'Email không khớp với email đã đăng ký của tài khoản.' });
    }

    // Rate limiting cho gửi email
    if (isRateLimited_('email_reset', username)) {
        return JsonResponse({ success: false, message: 'Bạn đang yêu cầu gửi OTP quá nhanh. Vui lòng thử lại sau 5 phút.' });
    }

    // Sinh mã OTP 6 số
    const otp = String(Math.floor(100000 + Math.random() * 900000));
    
    // Lưu OTP vào CacheService (hạn 5 phút)
    const cache = CacheService.getScriptCache();
    cache.put(`reset_otp_${username.toLowerCase()}`, otp, 300);

    // Gửi email
    const subject = "Mã xác nhận khôi phục mật khẩu - Quiz Lab";
    const body = `Xin chào,\n\nBạn đã yêu cầu khôi phục mật khẩu cho tài khoản "${username}" trên Quiz Lab.\n\nMã xác nhận (OTP) của bạn là: ${otp}\n\nMã OTP này có hiệu lực trong vòng 5 phút. Vui lòng không chia sẻ mã này cho bất kỳ ai.\n\nTrân trọng,\nBan quản trị Quiz Lab.`;
    const htmlBody = `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            <div style="background: linear-gradient(135deg, #0b6f87 0%, #5a62c8 100%); padding: 20px; text-align: center; color: white;">
                <h2 style="margin: 0; font-size: 24px; letter-spacing: 0.5px;">Quiz Lab Password Recovery</h2>
            </div>
            <div style="padding: 24px; background: #ffffff; color: #334155; line-height: 1.6;">
                <p>Xin chào,</p>
                <p>Bạn đã yêu cầu khôi phục mật khẩu cho tài khoản <strong>${username}</strong> trên hệ thống Quiz Lab.</p>
                <div style="margin: 30px 0; text-align: center;">
                    <div style="display: inline-block; padding: 12px 30px; background: #f1f5f9; border-radius: 6px; font-size: 28px; font-weight: bold; letter-spacing: 4px; color: #0b6f87; border: 1px dashed #cbd5e1;">
                        ${otp}
                    </div>
                </div>
                <p style="color: #ef4444; font-size: 14px;">Mã OTP này có hiệu lực trong vòng <strong>5 phút</strong>. Vui lòng không chia sẻ mã này cho bất kỳ ai.</p>
                <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;">
                <p style="font-size: 13px; color: #64748b; margin: 0;">Email này được gửi tự động từ hệ thống Quiz Lab. Vui lòng không trả lời thư này.</p>
            </div>
        </div>
    `;
    ...
    try {
        const resendPayload = {
            "from": "Quiz Lab <support@tsbyin.dev>",
            "to": [email],
            "subject": subject,
            "html": htmlBody,
            "text": body
        ...
        const res = UrlFetchApp.fetch("https://api.resend.com/emails", resendOptions);
        const resCode = res.getResponseCode();
        const resText = res.getContentText();

        if (resCode !== 200 && resCode !== 201) {
            Logger.log("Resend API failed: " + resText);
            return JsonResponse({ success: false, message: "Không thể gửi email lúc này. Dịch vụ gửi thư (Resend) phản hồi lỗi: " + resText });
        }
    } catch (e) {
        Logger.log("Email send exception: " + e.toString());
        return JsonResponse({ success: false, message: "Lỗi kết nối đến máy chủ gửi email. Lỗi: " + e.toString() });
    }

    registerRateLimitFailure_('email_reset', username);
...
function handleConfirmPasswordReset(data) {
    ...
    if (!username || !otp || !newPasswordHash) {
        return JsonResponse({ success: false, message: 'Vui lòng điền đầy đủ thông tin.' });
    }

    const cache = CacheService.getScriptCache();
    const cachedOtp = cache.get(`reset_otp_${username.toLowerCase()}`);

    if (!cachedOtp || cachedOtp !== otp) {
        return JsonResponse({ success: false, message: 'Mã OTP không chính xác hoặc đã hết hạn.' });
    }
    ...
    try {
        ...
        sheetsRef.users.getRange(matchedRowIndex + 1, userSchema.passwordHash + 1).setValue(newPasswordHash);
        
        cache.remove(`reset_otp_${username.toLowerCase()}`);
        clearRateLimitFailures_('email_reset', username);
        clearRateLimitFailures_('login', username);

        return JsonResponse({ success: true, message: 'Khôi phục mật khẩu thành công. Vui lòng đăng nhập lại.' });
    } catch (e) {
        return JsonResponse({ success: false, message: 'Lỗi máy chủ khi cập nhật mật khẩu.' });
    } finally {
        lock.releaseLock();
    }
}
...
function handleResetPasswordViaReferral(data) {
    const sheetsRef = getSheetsRef_();
    const username = String(data.username || '').trim();
    const referralCode = String(data.referralCode || '').trim().toUpperCase();
    const newPasswordHash = String(data.newPasswordHash || '').trim();

    if (!username || !referralCode || !newPasswordHash) {
        return JsonResponse({ success: false, message: 'Vui lòng điền đầy đủ thông tin.' });
    }

    const referralSheet = sheetsRef.referralCodes;
    if (!referralSheet) {
        return JsonResponse({ success: false, message: 'Hệ thống chưa cấu hình mã giới thiệu.' });
    }
    ...
    try {
        const referralSchema = getReferralCodesSchema_(referralSheet);
        const referralData = referralSheet.getDataRange().getValues();
        let isCodeValidForUser = false;

        for (let i = 1; i < referralData.length; i++) {
            const code = String(referralData[i][referralSchema.code] || '').trim().toUpperCase();
            if (code !== referralCode) {
                continue;
            }

            const usedBy = String(referralData[i][referralSchema.usedBy] || '').trim();
            const status = String(referralData[i][referralSchema.status] || '').trim().toLowerCase();
            
            if (status === 'used' && usedBy.toLowerCase() === username.toLowerCase()) {
                isCodeValidForUser = true;
                break;
            }
        }

        if (!isCodeValidForUser) {
            return JsonResponse({ success: false, message: 'Mã giới thiệu không khớp với mã đã dùng để đăng ký tài khoản này.' });
        }
        ...
        return JsonResponse({ success: true, message: 'Khôi phục mật khẩu bằng mã giới thiệu thành công.' });
    } catch (e) {
        return JsonResponse({ success: false, message: 'Lỗi máy chủ khi cập nhật mật khẩu.' });
    } finally {
        lock.releaseLock();
