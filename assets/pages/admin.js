let attemptChart;
        ...
        function getBulkActionConfig() {
            return [
                ...
                {
                    key: 'antiCheat',
                    label: 'Anti-Cheat (Cơ Bản)',
                    getter: (quiz) => isEnabled(quiz.antiCheat, false),
                    updater: (quizID, value) => stageQuizFieldChange(quizID, 'antiCheat', value)
                },
                {
                    key: 'antiCheatDevTools',
                    label: 'Anti-Cheat (DevTools)',
                    getter: (quiz) => isEnabled(quiz.antiCheatDevTools, false),
                    updater: (quizID, value) => stageQuizFieldChange(quizID, 'antiCheatDevTools', value)
                },
        ...
        function switchAdminView(view) {
            ...
            // Toggle panels
            const panels = ['overviewPanel', 'managePanel', 'functionPanel', 'importPanel', 'chartPanel', 'referralPanel', 'hashPanel'];
            panels.forEach(p => {
        ...
        async function loadAdminData(forceRefresh = false) {
            try {
                ...
                if (result.success) {
                    ...
                    showToast('Lỗi tải dữ liệu: ' + (result.message || 'Không xác định'), 'error');
                }
            } catch (error) {
                console.error('Error loading admin data:', error);
                showToast('Lỗi tải dữ liệu Admin. Vui lòng kiểm tra kết nối.', 'error');
            }
        }
            ...
            quizzes.forEach((quiz) => {
                ...
                card.insertAdjacentHTML('beforeend', `
                    ...
                    <div class="toggle-grid">
                        ...
                        <label class="toggle-label">
                            <span>Anti-Cheat (Cơ Bản)</span>
                            <input type="checkbox" class="toggle-switch" 
                                ${isEnabled(quiz.antiCheat, false) ? 'checked' : ''}
                                onchange="updateAntiCheat(decodeURIComponent('${encodedQuizID}'), this.checked, this)">
                            <span class="toggle-slider"></span>
                        </label>

                        <label class="toggle-label">
                            <span>Anti-Cheat (DevTools)</span>
                            <input type="checkbox" class="toggle-switch" 
                                ${isEnabled(quiz.antiCheatDevTools, false) ? 'checked' : ''}
                                onchange="updateAntiCheatDevTools(decodeURIComponent('${encodedQuizID}'), this.checked, this)">
                            <span class="toggle-slider"></span>
                        </label>
        ...
        function updateAntiCheat(quizID, antiCheat, toggleEl) {
            if (adminBatchSaving || isAdminToggleBusy(quizID, 'antiCheat')) {
                if (toggleEl) toggleEl.checked = !antiCheat;
                return;
            }
            stageQuizFieldChange(quizID, 'antiCheat', antiCheat);
        }

        function updateAntiCheatTabSwitch(quizID, antiCheatTabSwitch, toggleEl) {
            if (adminBatchSaving || isAdminToggleBusy(quizID, 'antiCheatTabSwitch')) {
                if (toggleEl) toggleEl.checked = !antiCheatTabSwitch;
                return;
            }
            stageQuizFieldChange(quizID, 'antiCheatTabSwitch', antiCheatTabSwitch);
        }

        function updateAntiCheatFullscreen(quizID, antiCheatFullscreen, toggleEl) {
            if (adminBatchSaving || isAdminToggleBusy(quizID, 'antiCheatFullscreen')) {
                if (toggleEl) toggleEl.checked = !antiCheatFullscreen;
                return;
            }
            stageQuizFieldChange(quizID, 'antiCheatFullscreen', antiCheatFullscreen);
        }

        function updateAntiCheatDevTools(quizID, antiCheatDevTools, toggleEl) {
            if (adminBatchSaving || isAdminToggleBusy(quizID, 'antiCheatDevTools')) {
                if (toggleEl) toggleEl.checked = !antiCheatDevTools;
                return;
            }
            stageQuizFieldChange(quizID, 'antiCheatDevTools', antiCheatDevTools);
        }
        ...
        document.getElementById('hashClearBtn').addEventListener('click', () => {
            ...
            document.getElementById('hashOutputText').value = '';
            document.getElementById('hashFeedback').textContent = '';
            document.getElementById('hashFeedback').className = 'feedback hash-feedback';
            document.getElementById('hashInputText').focus();
        });
        ...
        document.getElementById('hashGenerateBtn').addEventListener('click', async () => {
            ...
            const outputText = document.getElementById('hashOutputText');
        ...
        document.getElementById('hashCopyBtn').addEventListener('click', async () => {
            const outputText = document.getElementById('hashOutputText');
            const feedback = document.getElementById('hashFeedback');
            if (!outputText.value) {
                feedback.className = 'feedback incorrect';
        ...
        document.getElementById('hashInputText').addEventListener('keypress', (e) => {
            if (e.key === 'Enter') document.getElementById('hashGenerateBtn').click();
        });
